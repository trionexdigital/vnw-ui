      ...
      {success && (
        <div className="vnw-card mb-5 flex items-center gap-3 border-emerald-500/40 p-4">
          ...
          <div><div className="font-semibold text-foreground">Payment successful!</div>
            <div className="text-sm text-muted-foreground">Your VIP number order is confirmed and its transfer progress is shown below.</div></div>
        </div>
      )}
      ...
      <div className="vnw-card p-6">
        ...
        <div className="mt-5 divide-y divide-card-border">
          {order.items?.map((it: any) => (
            <div key={it.order_item_id} className="flex flex-wrap items-center justify-between gap-3 py-3">
              <div>
                <div className="vnw-card-numeral text-lg text-foreground">{it.display_number}</div><div className="text-xs text-muted-foreground">{getPrimaryCategory(it)?.name || 'Unique'} · <StatusBadge status={it.item_status} /></div>
              </div>
              <div className="text-right"><Money value={it.line_total ?? it.price}/></div>
            </div>
          ))}
        </div>
