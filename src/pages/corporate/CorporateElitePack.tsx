...
import { CorporatePackCard, PACK_TYPE_LABELS } from '@/pages/home/components/MarketplaceSections';
import { Loader } from '@/shared/components/ui-bits';

const types: CorporatePackType[] = ['MIXED', 'SIMILAR_START', 'SIMILAR_END', 'SIMILAR_BOTH'];

export default function CorporateElitePack() {
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    siteAPI.corporatePacks({ pack_type: type, size })
    ...
    <main className="bg-background text-foreground">
      <section className="relative overflow-hidden border-b border-border bg-card px-4 py-14 sm:px-6 lg:px-8">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.2fr_.8fr] lg:items-center">
          <div><span className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-black uppercase text-primary"><Building2 className="h-4 w-4" /> Corporate Elite Pack</span><h1 className="mt-4 max-w-3xl text-4xl font-black sm:text-5xl">A coordinated VIP identity for every key connection</h1><p className="mt-4 max-w-2xl leading-7 text-muted-foreground">Select a matching style and pack size. Live available numbers are grouped automatically, with current pricing and availability checked before they reach your cart.</p></div>
          <div className="rounded-3xl border border-primary/25 bg-background p-6 shadow-xl"><Users2 className="h-9 w-9 text-primary" /><h2 className="mt-3 text-xl font-black">Designed for teams</h2><div className="mt-4 space-y-3 text-sm text-muted-foreground">{['Live inventory and pricing', 'One complete cart action', 'Existing secure checkout', 'Clear operator details'].map((label) => <div key={label} className="flex items-center gap-2"><span className="grid h-6 w-6 place-items-center rounded-full bg-primary/10 text-primary"><Check className="h-3.5 w-3.5" /></span>{label}</div>)}</div></div>
        </div>
      </section>
      ...
      <section className="px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-7">
            {loading ? <Loader variant="cards" /> : packs.length ? <div className="grid gap-4 lg:grid-cols-2">{packs.map((pack) => <CorporatePackCard key={pack.pack_id} pack={pack} />)}</div> : <div className="rounded-3xl border border-border bg-card px-6 py-14 text-center"><Building2 className="mx-auto h-10 w-10 text-primary" /><h2 className="mt-3 text-xl font-black">No matching live pack right now</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted-foreground">Try another matching style or a smaller pack size. Results are generated only from numbers that are currently available.</p><Link to="/contact?subject=corporate-elite-pack" className="btn-gold mt-5">Ask for assistance <ArrowRight className="h-4 w-4" /></Link></div>}
          </div>
        </div>
      </section>
    </main>
