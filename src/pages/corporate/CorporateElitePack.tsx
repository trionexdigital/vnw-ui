    ...
    <main className="bg-background text-foreground">
      ...
      <section className="px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-7">
            {loading ? <Loader variant="cards" /> : packs.length ? <div className="grid gap-5 lg:grid-cols-2">{packs.map((pack) => <FamilyPackCard key={pack.pack_id} pack={pack} wishedIds={wishedIds} onWishlistChange={updateWishlist} />)}</div> : <div className="rounded-3xl border border-border bg-card px-6 py-14 text-center"><Building2 className="mx-auto h-10 w-10 text-primary" /><h2 className="mt-3 text-xl font-black">No matching live Family Pack right now</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted-foreground">Try another matching style or a smaller pack size. Results are generated only from numbers that are currently available.</p><Link to="/contact?subject=family-pack" className="btn-gold mt-5">Ask for assistance <ArrowRight className="h-4 w-4" /></Link></div>}
          </div>
        </div>
      </section>
    </main>
