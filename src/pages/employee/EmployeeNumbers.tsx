...
export default function EmployeeNumbers() {
  ...
  const [f, setF] = useState<any>({ display_number: '', operator: 'Any', mrp: '', offer_price: '' });
  ...
  const createNumber = async (e: React.FormEvent) => {
    ...
    setShowForm(false); setF({ display_number: '', operator: 'Any', mrp: '', offer_price: '' });
  };
        ...
        <Table head={['Number', 'Category', 'Price', 'Status', 'Actions']}>
          {rows.map((n) => (
            <tr key={n.number_id} className="border-b border-card-border last:border-0">
              ...
              <td className="px-4 py-3 text-muted-foreground">{getPrimaryCategory(n)?.name || 'Unique'}</td>
