...
export default function Cart() {
  ...
  const remove = async (it: any) => { await cartAPI.remove(it.number_id); await refreshCounts(); load(); };
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
          <div className="space-y-3">
            {data.items.map((it: any) => (
              <div key={it.number_id} className="vnw-card flex flex-wrap items-center justify-between gap-4 p-4">
                <div>
                  <Link to={`/number/${it.number_id}`} className="vnw-card-numeral text-xl text-foreground">{it.display_number}</Link>
